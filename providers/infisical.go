...
func (p *InfisicalProvider) loginUniversalAuth(ctx context.Context) (string, int, error) {
	form := url.Values{}
	form.Set("clientId", p.config.ClientID)
	form.Set("clientSecret", p.config.ClientSecret)

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.config.SiteURL+infisicalLoginPath, strings.NewReader(form.Encode()))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		return "", 0, fmt.Errorf("infisical login returned status %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
