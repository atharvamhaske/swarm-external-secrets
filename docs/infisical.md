...
Local/CI smoke coverage lives in `scripts/tests/smoke-test-infisical.sh`.

- **Default (no Infisical account):** uses `scripts/tests/mock-infisical-server`
- **Real Infisical:** set `INFISICAL_SMOKE_TOKEN` and/or `INFISICAL_SMOKE_CLIENT_ID` + `INFISICAL_SMOKE_CLIENT_SECRET`, plus `INFISICAL_PROJECT_ID` (and optionally `INFISICAL_ENVIRONMENT` / `INFISICAL_SITE_URL`)
...
- Uses Infisical REST API directly (no SDK dependency), matching the Doppler provider approach.
